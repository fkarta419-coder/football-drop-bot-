# main.py
# CRYPTO EMPIRE — ИСПРАВЛЕННАЯ ВЕРСИЯ
# Добавлено: Ежедневный бонус, Майнинг, Инвестиции, Полное казино
...
async def init_db():
    async with aiosqlite.connect(DB) as db:
        ...
        CREATE TABLE IF NOT EXISTS casino_stats (
            ...
            roulette_best_win INTEGER DEFAULT 0,
            slots_best_win INTEGER DEFAULT 0
        )
        """)
...
async def require_subscription(message):
    ...
    await message.answer(
        ...
        f"Подпишись на канал, чтобы играть!",
...
async def check_sub_callback(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.answer("❌ Ошибка проверки.", reply_markup=subscribe_keyboard())
...
async def dice_callback(callback: CallbackQuery):
    ...
    # Ставка 1000, число 50 (для демо)
    bet = 1000
    number = random.randint(1, 100)
    guess = random.randint(1, 100)  # AI угадывает
...
async def main():
    ...
    print(f"👑 OWNER: @{OWNER}")
    print("📢 КАНАЛ: https://t.me/+MHTPcaFy2j5lOWMy")
    print("=" * 60)
